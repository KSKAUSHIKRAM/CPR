...
def ai_normalize_input(text: str, location: str = "home", time_phase: str = "unspecified") -> str:
    """
    Use OpenAI GPT to normalize colloquial or misspelled AAC input
    into a short, clear English sentence suitable for assistive communication.
    This is an optional AI layer that complements rule-based normalization.
    """
    ...
    prompt = f"""
    You are an AAC text normalizer that rewrites colloquial or misspelled user inputs
    into short, natural English sentences suitable for assistive communication. Also give me the normalized text according to the number of input text.
    Input: "{text}"
    Context: location={location}, time_phase={time_phase}.
    Output only the corrected, natural sentence.
    """
    ...
    try:
        ...
        print(f"[AI NORMALIZER] '{text}' → '{normalized}'")
        return normalized
