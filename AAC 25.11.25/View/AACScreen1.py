...
import os, requests, threading, time, socket, subprocess, shelve, tempfile, wave, audioop, json, pygame, io
...
def is_grammatically_valid(sent):
    ...
    tags   = [t.tag_ for t in doc]  # POS tags (VB, VBG, etc.)

    # ❌ Reject "was" followed by base verb (VB) → e.g. "I was wait"
    for i, t in enumerate(tokens):
        if t == "was" and i+1 < len(tokens):
            if tags[i+1] == "VB":
                return False

    # ❌ Reject "want" followed by wrong verb form → e.g. "I want waited"
    for i, t in enumerate(tokens):
        if t == "want" and i+1 < len(tokens):
            if tags[i+1] not in {"VB"}:  # allow only infinitive/base form
                return False
...
def all_tokens(sentence: str):
    """Tokenize EVERYTHING (including function words)."""
    return WORD_RE.findall(sentence.lower())
    
def normalize_input(text: str) -> str:
        """
        Very lightweight spelling fixes / expansions for AAC inputs.
        """
        replacements = {
        ...
        for tok in tokens:
            if tok in replacements:
                # pick the most common/intended candidate
                new_tokens.append(replacements[tok][0])
            else:
                new_tokens.append(tok)
        return " ".join(new_tokens)

# Kept if you still need content-only tokens elsewhere
FUNCTION_WORDS = {
...
def get_arasaac_image_url(word):
    """Tiny cache + fetch for a single token keyword."""
    ...
    with shelve.open(CACHE_DB) as cache:
        if key in cache:
            return cache[key]
        url = f"https://api.arasaac.org/api/pictograms/en/search/{key}"
        try:
...
class AACScreen(Screen):
    def __init__(self, location_label="Default", **kwargs):
        ...
        self._built = False
        Clock.schedule_once(lambda dt: self.build_ui())  # build once on main thread
    ...
    def recommend(self, _): self.db_help.recommend(self.location_label)
    def clear_all(self, _): self.text_input.text = ""; self.result_grid.clear_widgets()
    def exit_app(self, _): App.get_running_app().stop()
    def update_time(self, dt): self.task_label.text = f"Location: {self.location_label} | Time: {time.strftime('%H:%M:%S')}"
